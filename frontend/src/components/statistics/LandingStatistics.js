...
const LandingStatistics = ({ selectedGame, onGameChange }) => {
  // Używamy hooka do pobierania danych
  const { data: stats, loading, error, lastUpdated, refreshData } = useLottoData(selectedGame || 'lotto');
  ...
  // Dane są już pobierane przez hook useLottoData
  
  // Debug logging
  useEffect(() => {
    console.log('🔍 [LandingStatistics] Debug info:', {
      selectedGame: selectedGame || 'lotto',
      loading,
      error,
      stats: stats ? 'dane dostępne' : 'brak danych',
      lastUpdated
    });
  }, [selectedGame, loading, error, stats, lastUpdated]);
          ...
          <button 
            onClick={async () => {
              ...
              try {
                ...
                // Używamy hooka do odświeżenia danych
                await refreshData();
          ...
          <button 
            onClick={async () => {
              ...
              try {
                ...
                alert(`✅ Wszystkie statystyki zostały zaktualizowane!`);
              } catch (error) {
